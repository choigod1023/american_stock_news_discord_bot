...
**중복 제거 및 품질 관리 (핵심!):**
1. **중복 내용 제거**: 유사한 제목이나 내용의 뉴스는 하나로 통합하여 언급
2. **알맹이 있는 뉴스만 선별**: 
   - 빈 내용이나 의미 없는 뉴스 제외
   - 구체적인 정보(숫자, 종목명, 인사이트)가 있는 뉴스 우선
   - 좋아요/조회수가 높거나 중요 주식 관련 뉴스 우선
3. **다양성 유지**: 하나의 회사나 이슈에 집중하지 말고 다양한 섹터/주제로 분산

**요구사항 (간결하게):**
1. **시장 현황** (1-2문장): 나스닥 방향성 + 공포탐욕지수 해석
2. **주요 이슈** (2-3개): 가장 중요한 상승/하락 요인만 간단히, 중복 없이
...
위 정보를 바탕으로 **800자 이내의 간결하고 가독성 좋으며, 중복 없는 시장 요약**을 작성하세요.
"""
        return prompt
